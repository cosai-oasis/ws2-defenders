...
| **Event Sequence Continuity** | SHOULD | Per-session monotonic sequence number enabling gap and reordering detection. The stream **SHOULD be hash-chained**, and where chained the **chain head SHOULD be periodically signed and published outside the emitter's trust domain**. Where externally witnessed, the verification material SHOULD remain resolvable from the retained telemetry (see [Appendix D.5](#d5-context-propagation-sampling--privacy-three-operational-traps)). That is the minimum that lets a verifier distinguish a gap from a suppression, and the only form that detects `TA-17`, where the emitter itself is redirected. | `TA-17`,`AOC-01`,`AOC-10`,`AOC-07` |
