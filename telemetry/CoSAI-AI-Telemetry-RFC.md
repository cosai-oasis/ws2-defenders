...
55. **OpenTelemetry Audit Data Model (draft).** The `audit.*` attribute namespace for tamper-evident audit records: actor, action, outcome, integrity (hash chain, signatures, canonicalization), and sequencing. Developed in the open under the Audit Logging SIG proposal ([#2409](https://github.com/open-telemetry/community/pull/2409)); not adopted into the specification. Verified against `apeirora/opentelemetry-specification` branch `auditing`, `specification/audit/data-model.md` @ `ce273353` (17 September 2026); the branch is under active development, so the SHA is the only stable anchor. Referenced in [Appendix D](#appendix-d-implications-for-opentelemetry-the-instrumentation-bridge).
...
**Canonicalization is a prerequisite for hash-first, and it must be declared.** A content hash or an event fingerprint correlates across producers only if every producer hashes the same logical bytes. The OpenTelemetry audit data model draft mandates RFC 8785 (JCS) [[55]](#standards--frameworks); OCSF's `attestation.fingerprint` instead *declares* its serialization (`serialization_id` plus a free-text `serialization`) so that non-JCS producers can declare it; CPEX's audit seam ([cpex#166](https://github.com/contextforge-org/cpex/pull/166)) hashes a sorted-key JSON that is documented as not RFC 8785. Two conformant implementations with different canonicalizations produce different digests for the same event, and the integrity claim degrades silently from "verify" to "trust the producer". Requirement: a producer that emits a hash of content or of an event MUST declare the canonicalization used (JCS by default), and consumers MUST NOT compare digests across producers whose declared canonicalizations differ. OCSF carries the declaration on `attestation.fingerprint`; on the OTel side the declaration rides on `audit.integrity.canonicalization` [[55]](#standards--frameworks), present in the draft since its September 9 revision — the remaining work is carrying it through to adoption, not adding it.
