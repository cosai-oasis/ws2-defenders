...
**A hash is evidence only if a second party can recompute it.** The canonicalization the digest is taken over MUST be declared, either by the deployment or by the carrier ([D.5](#d5-context-propagation-sampling--privacy-three-operational-traps)). Two emitters that hash the same tool call under different serializations produce different digests, and the field degrades silently from evidence to a correlation key that only works within one producer. One existing form of such a declaration is the SCITT **Canonical Payload Binding** [[55]](#standards--frameworks), an individual Internet-Draft (not adopted by the SCITT working group at the time of writing) that fixes the canonicalization algorithm, hash function, and exclusion set once per payload class so that independent emitters and verifiers recompute the same digest.
...
55. **Canonical Payload Binding**: Mih, S. & Sokolov, A. *Canonical Payload Binding: A Signed Statement Construction Profile.* Internet-Draft draft-mih-sokolov-scitt-payload-binding-03, Work in Progress, September 2026. Individual submission; not adopted by the SCITT working group at the time of writing. <https://datatracker.ietf.org/doc/draft-mih-sokolov-scitt-payload-binding/>
